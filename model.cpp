...
int getBoneLinesFromSkeleton(CalSkeleton *skel, float *pLines)
{
	int nrLines;
	nrLines = 0;
...
int getBonePointsFromSkeleton(CalSkeleton*skel, float *pPoints)
{
	int nrPoints;
	nrPoints = 0;
...
void Model::onRender()
{
  ...
  //renderSkeleton();
  //renderBoundingBox();
  renderMesh(false, true); //theMenu.isWireframe(), theMenu.isLight());
